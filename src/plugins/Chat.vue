...
let html = $ref(''), chat = $ref([]), resp = $ref(null)
let scroll = $ref()

let ok = $computed(() => resp.match(/\S/))

setListener(msg => { // listen from ASlide
  if (msg.slide) {
    ...
    <div class="w-full max-w-screen-md mx-auto h-full flex flex-col justify-center bg-gray-100 shadow-md">
      <div class="flex flex-col items-end w-full grow overflow-y-auto p-4">
        <template v-for="msg in chat">
          <div class="m-1 p-1 px-2 border rounded-lg break-all w-fit max-w-[80%]" :class="msg.self ? 'bg-sky-600 text-white' : 'bg-white self-start'">{{ msg.content }}</div>
        </template>
        <div ref="scroll"></div>
      </div>
      <div class="flex">
        <input class="m-2 mr-0 p-2 grow" v-model="resp" @keyup.enter="response">
