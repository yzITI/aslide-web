    ...
    <div class="flex flex-col w-60 bg-gray-700 text-white"><!-- channel control -->
      <div class="w-full p-2 flex flex-col"><!-- control panel -->
        ...
        <div class="flex items-center my-1">
          ...
          <div class="rounded-full h-4 w-4 mx-2" :class="channel.time ? 'bg-green-600' : 'bg-gray-500'" />
          <div class="text-gray-300 text-xs">{{ channel.time ? parseTime(channel.time) : 'unconnected' }}</div>
        </div>
      </div>
      <div class="w-full grow overflow-auto p-2"><!-- session list -->
        <div v-for="(s, id) in channel.sessions">{{ id }}</div>
      </div>
    </div>
  </div>
</template>